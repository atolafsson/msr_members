...
func NotAdminUser(w http.ResponseWriter, r *http.Request) {
	...
	<p><a href='/members/all'>Back to Members</a></p>
	</body></html>`)
}
...
func Index(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprint(w, "<p><a href='/members/all'>Manage Members</a></p>")
}

func Members(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	userType, _ := vars["userType"]
	log.Println("Members: user type=" + userType + " Ip=" + r.RemoteAddr)
	userType = strings.ToLower(userType)
	if !(userType == "all" || userType == "active") {
		userType = "all"
	}
	PrintFilterHeader(w, "MSR Current Members", "[[1,0]]", false)
	fmt.Fprintf(w, `<center>Click here to <a href='/editmember/0'>Add new Member</a>&emsp;&emsp;To filter members:
	<a href='/members/all'>All members</a>&emsp;<a href='/members/active'>Active members</a></center></div>`)
...
func EditMember(w http.ResponseWriter, r *http.Request) {
    ...
    <tr><td></td>
				...
				<label>&nbsp;&nbsp;0=Member, 1=Prospect, 2=On Leave, 3=Gone</label></td>
			</tr><tr>
...
func SaveMember(w http.ResponseWriter, r *http.Request) {
	...
	if IsAdminUser(r) {
		...
		if mID == 0 {
			...
			http.Redirect(w, r, "/members/all", http.StatusFound)
		}
	} else {
		NotAllowed(w, r)
	}
